...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, learning_frequency = 10, lr_actor = LR_ACTOR, lr_critic = LR_CRITIC, weight_decay = WEIGHT_DECAY):
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        reward_GAE = np.sum(self.GAE_lambdas[:min(20,len(reward))] * reward[-min(20,len(reward)):])
            
        # Save experience / reward
        self.memory.add(state, action, reward_GAE, next_state, done)
        self.steps += 1
    ...
    def act(self, state, add_noise=True):
        ...
        if add_noise:
            action += self.noise.sample() * self.noise_ratio
